...
public class PlayerRotationHandler implements WorldRenderEvents.BeforeEntities, WorldRenderEvents.AfterEntities {
    ...
    @Override
    public void beforeEntities(WorldRenderContext event) {
        PoseStack stack = event.matrixStack();
        MultiBufferSource buffers = event.consumers();
        ...
        if (riding instanceof BeachFloatEntity) {
            ...
            stack.pushPose();
            stack.mulPose(Vector3f.YP.rotationDegrees(-(floaty.yRotO + (event.tickDelta() * (floaty.getYRot() - floaty.yRotO)))));
            stack.translate(0, 1.55, 1.55);
            stack.mulPose(Vector3f.XN.rotationDegrees(90));

            // Cancel out player camera rotation
            float f = interpolateAndWrap(p.yBodyRot, p.yBodyRotO, event.tickDelta());
        ...
        if (riding instanceof SeaTurtleEntity) {
            ...
            // Cancel out player camera rotation
            float pitch = interpolateAndWrap(turtle.getXRot(), turtle.xRotO, event.tickDelta());
            float yaw = interpolateAndWrap(turtle.yHeadRot, turtle.yHeadRotO, event.tickDelta());
    ...
    @Override
    public void afterEntities(WorldRenderContext event) {
        Player p = Minecraft.getInstance().player;
